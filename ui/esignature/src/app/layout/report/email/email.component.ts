...
export class EmailComponent extends Softcafe implements OnInit, Service, AfterViewInit {
  ...
  onSearch() {
    ...
    this.cs.sendRequest(this, ActionType.SEARCH, ContentType.MailTracker, 'search', payload);
  ...
  loadMailTracker(){
    ...
    this.cs.sendRequest(this, ActionType.SELECT, ContentType.MailTracker, 'select', payload);
  }
...
debugger
    ...
    var branchId = this.searchForm.value.branchId;
  ...
  onViewClick(e, args) {
    ...
    this.cs.sendRequest(this, ActionType.RAW_MESSAGE, ContentType.MailTracker, 'RAW_MESSAGE', payload);
  ...
  innerHtmlBody: Formatter = (row: number, cell: number, value: any, columnDef: Column, dataContext: any, grid?: any) => {
    ...
    return '<span title="edit"  style="font-size:14px;">'+dataContext.body+'</span>'
  };
  innerHtmlSubject: Formatter = (row: number, cell: number, value: any, columnDef: Column, dataContext: any, grid?: any) => {
    
    return '<span title="edit"  style="font-size:14px;">'+dataContext.subject+'</span>'
  };
  ...
  prepareGrid() {
    ...
    this.columnDefinitions = [
      ...
      // {
      //   id: 'mailTrakerId', name: 'Mail Traker Id', field: 'mailTrakerId',
      ...
      {
        id: 'subject', name: 'Subject', field: '',formatter: this.innerHtmlSubject,
        sortable: true, type: FieldType.text,
      ...
      // },
      /* {
        id: 'mailGroup', name: 'Mail Group', field: 'mailGroup',
        sortable: true, type: FieldType.text,
        filterable: true, filter: { model: Filters.inputText, },
        minWidth: 150
      }, */
    ...
    this.dataset = this.emailList;
    // this.body = this.emailList;
  }

  buildGridOptions() {
    var menuOrder = 60;
    var option: GridOption = {
      datasetIdPropertyName: "mailTrakerId",
  ...
  // }
   if (res.header.referance == 'search') {
    ...
    if (res.payload.length > 1) {
      ...
      this.emailList =res.payload;
      this.dataset = res.payload;
    } else {
      this.emailList =null;
      this.dataset = null;
      Swal.fire({ title: "No Data Found!", icon: 'warning', toast: true, timer: 5000 });
    }
   
  }
   else if (res.header.referance == "select") {

      console.log("select",res.payload);
      this.emailList =res.payload;
      this.dataset = res.payload;
