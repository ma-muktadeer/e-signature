...
export class SignatureReportComponent extends Softcafe implements OnInit, Service, AfterViewInit {
  ...
  prepareGrid() {
    ...
    this.columnDefinitions = [
      ...
      // {
      //   id: 'userCreateKey', name: 'User Create Key', field: 'userCreateKey',
      //   sortable: true, type: FieldType.number,
      ...
      // },
      {
        ...
        sortable: true, type: FieldType.text, filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 190
      },
      {
      ...
      {
        id: 'createDate', name: 'PA Generation Date & Time', field: 'createDate',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 180,
      },
      ...
      {
        id: 'signatureCreateDate', name: 'Signature Upload Date & Time', field: 'signatureCreateDate',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 180,
      },
      ...
      {
        id: 'authorizeTime', name: 'Signature Checker Date & Time', field: 'authorizeTime',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 190,
      },
      ...
      {
        id: 'effectiveDate', name: 'Effective Date', field: 'effectiveDate',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 180,   formatter: (row, cell, value) => value ? value.split(' ')[0] : value,
      },
      {
      ...
      {
        id: 'cancelCircularUploadDate', name: 'Cancel Circular Upload Date & Time', field: 'cancelCircularUploadDate',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 170,
      },
  ...
  loadData(fromDate, toDate, pointer) {
    ...
    this.cs.execute(ActionType.ACTION_SIG_SEARCH_REPORT, ContentType.ReportSearch, payload)
      .subscribe((res: any) => {
        console.log(res)
        debugger
        this.searchReport = res.payload.itemsSignatureReport.map(item => ({
          ...item,
          pointer: pointer
        }));
