...
@Component({
  selector: 'app-news-feed-others-report',
  templateUrl: './news-feed-others-report.component.html',
  styleUrls: ['./news-feed-others-report.component.scss']
})
export class NewsFeedOthersReportComponent extends Softcafe implements OnInit, Service, AfterViewInit {
  now = new Date()
  fromDate = new Date()
  toDate = new Date()
  institutionId = null;
  ...
  prepareGrid() {
    ...
    this.columnDefinitions = [
...

      {
        ...
        sortable: true, type: FieldType.text, formatter: (row, cell, value, columnDef, dataContext) => blockToCamel(dataContext.activityType),
        filterable: isFilterable, filter: { model: Filters.inputText, },
      },
      {
        id: 'actionTime', name: 'Action Time', field: 'actionTime',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
      },
      {
        id: 'activity', name: 'Activity Time', field: 'activity',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
      },
  ...
  loadData(fromDate, toDate, pointer) {
    ...
    this.cs.execute(ActionType.ACTION_NEWS_FEED_OTHERS_REPORT, ContentType.ReportSearch, payload)
      .subscribe((res: any) => {
        console.log(res)
        this.searchReport = res.payload.itemsNewsFeedReport.map(item => ({
          ...item,
          pointer: pointer
        }));
