...
export class ExceptionReportComponent extends Softcafe implements OnInit, Service, AfterViewInit {
  now = new Date()
  fromDate = new Date()
  toDate = new Date()
  institutionId = null;
  ...
  prepareGrid() {
    ...
    this.columnDefinitions = [
      ...
      {
        id: 'branchName', name: 'Branch Name', field: 'branchName',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 150
      },
...

      {
        ...
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, }, formatter: (row: number, cell: number, value: any, columnDef: Column, dataContext: any) => blockToCamel(dataContext?.userStatus),
        minWidth: 180
      },
      ...
      {
        id: 'attemptStatus', name: 'Attempt Status', field: 'attemptStatus',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 150
      },
      {
        id: 'attemptLogin', name: 'Attempt Login', field: 'attemptLogin',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 200
      },
  ...
  loadData(fromDate, toDate, pointer) {
    ...
    this.cs.execute(ActionType.ACTION_EXCEPTION_REPORT, ContentType.ReportSearch, payload)
      .subscribe((res: any) => {
        console.log(res)
        this.searchReport = res.payload.itemsExceptionReportViews.map(item => ({
          ...item,
          pointer: pointer
        }));
