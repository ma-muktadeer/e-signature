...
@Component({
  selector: 'app-signature-history-report',
  templateUrl: './signature-history-report.component.html',
  styleUrls: ['./signature-history-report.component.scss']
})
export class SignatureHistoryReportComponent extends Softcafe implements OnInit, Service, AfterViewInit {
  now = new Date()
  fromDate = new Date()
  toDate = new Date()
  institutionId = null;
  ...
  prepareGrid() {
    ...
    this.columnDefinitions = [
      ...
      {
        id: 'userName', name: 'PA Holder Name', field: 'userName',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 210
      },
      ...
      {
        id: 'updateImageDate', name: 'Updated image date & Time.', field: 'updateImageDate',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 170
      },
  ...
  loadData(fromDate, toDate, pointer) {
    ...
    this.cs.execute(ActionType.ACTION_SIG_HISTORY, ContentType.ReportSearch, payload)
      .subscribe((res: any) => {
        console.log(res)
        this.searchReport = res.payload.itemsSignHistryReport.map(item => ({
        ...item,
        pointer: pointer 
      }));
