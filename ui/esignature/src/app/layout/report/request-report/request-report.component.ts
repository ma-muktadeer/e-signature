...
import { Column, GridOption, AngularGridInstance, PaginationService, FieldType, Filters, MenuCommandItem, ServicePagination } from 'angular-slickgrid';
...
export class RequestReportComponent extends Softcafe implements OnInit, Service, AfterViewInit {
  now = new Date()
  fromDate = new Date()
  toDate = new Date()
  institutionId = null;
  ...
  prepareGrid() {
    ...
    this.columnDefinitions = [
      ...
      {
        id: 'requesterName', name: 'Requester Name', field: 'requesterName',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 170
      },
      ...
      // },
      {
        ...
        minWidth: 170,
        formatter: (row: number, cell: number, value: string) => {
          // Ensure value exists and format it
          return value ? value.split('.')[0] : ''; // Removes milliseconds
        }
      },
      {
      ...
      },
      {
        ...
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, }, formatter: (row: number, cell: number, value: any, columnDef: Column, dataContext: any) => blockToCamel(dataContext?.requestStatus),
        minWidth: 150
      },
      {
        id: 'requestType', name: 'Request Type', field: 'requestType',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, }, formatter: (row: number, cell: number, value: any, columnDef: Column, dataContext: any) => blockToCamel(dataContext?.requestType),
        minWidth: 170
      },
      {
        id: 'publicLinkSend', name: 'Sending Email', field: 'publicLinkSend',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 230
      },
      ...
      {
        id: 'checkedBy', name: 'Checker Name', field: 'checkedBy',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 170
      },
      {
        id: 'checkerDate', name: 'Checker Date & Time', field: 'checkerDate',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 170
      },
      {
        id: 'completedBy', name: 'Approver Name', field: 'completedBy',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 170
      },
      {
        id: 'completeDateRequest', name: 'Approver Date & Time', field: 'completeDateRequest',
        sortable: true, type: FieldType.text,
        filterable: isFilterable, filter: { model: Filters.inputText, },
        minWidth: 170
      },
  ...
  loadData(fromDate, toDate, pointer) {
    ...
    this.cs.execute(ActionType.ACTION_REQUEST_REPORT, ContentType.ReportSearch, payload)
      .subscribe((res: any) => {
        console.log(res)
        this.searchReport = res.payload.itemsRequestView.map(item => ({
          ...item,
          pointer: pointer
        }));
